...
function initTypingAnimation() {
  ...
  var typing = function typing(el, sentence) {
    ...
    if (lang === "en-US") {
      _toConsumableArray(sentence).forEach(function (_char, index) {
        setTimeout(function () {
          target.innerHTML += _char === "\n" ? "<br>" : _char;
        }, 60 * index);
      });
    } else {
      _toConsumableArray(sentence).forEach(function (_char2, index) {
        setTimeout(function () {
          target.innerHTML += _char2 === "\n" ? "<br>" : _char2;
        }, 100 * index);
      });
    }
  };
  setTimeout(function () {
    if (lang === "en-US") {
      typing(".js-typing--en", "Coding that transforms experience\nthrough the power of UX.");
    } else {
      typing(".js-typing--ja", window.innerWidth <= 419 ? "魅せ方ひとつで、\n体験が変わる\nUXをデザインするコーディング" : "魅せ方ひとつで、体験が変わる\nUXをデザインするコーディング");
    }
  }, 1000);
}
...
function initScrollAnimation() {
  ...
  if (windowTop > troubleOffsetTop - headerHeight) {
    $troubleElement.find(".p-top-troubles__item").each(function (index) {
      var _this = this;
      setTimeout(function () {
        return jquery__WEBPACK_IMPORTED_MODULE_0___default()(_this).addClass("is-fadeIn");
      }, index * 200);
    });
  }
...
function animatePromiseSection() {
  if (jquery__WEBPACK_IMPORTED_MODULE_0___default()(window).width() <= 1024) return;
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()(".p-top-promise__img").each(function (index) {
    ...
    var $item = jquery__WEBPACK_IMPORTED_MODULE_0___default()(".p-top-promise__item").eq(index);
    if (index === 0) {
      imgBottom < windowTop + headerHeight ? $item.removeClass("is-active") : $item.addClass("is-active");
    }
    if (index === 1 || index === 2) {
      if (imgTop > windowTop + headerHeight || imgBottom < windowTop + headerHeight) {
    ...
    if (index === 3) {
      imgTop <= windowTop + headerHeight ? $item.addClass("is-active") : $item.removeClass("is-active");
    }
  });
}
